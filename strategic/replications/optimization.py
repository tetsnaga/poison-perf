...
def logistic_regression(X_orig, Y_orig, lam, method, tol=1e-7, theta_init=None):
    ...
    ----------
        ...
        method: string
            optimization method: 'RRM' for returning the exact solution and 'RGD' for performing a single gradient descent step on the parameter vector
        tol: float
    ...
    if method == 'RRM':
        ...
        # Default to zero initialization when theta_init is not provided
        if theta_init is None:
            theta_init = np.zeros(d)
        eta_init = 2 / (smoothness + 2 * lam)

    else:
        print('method must be RRM or RGD')
        raise ValueError
    ...
    while gap > tol and not is_rgd:
        ...
        if loss > prev_loss and method == 'RRM':
            eta = eta * .1
            gap = 1e30
            continue
        else:
